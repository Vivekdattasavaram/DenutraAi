...
class AdaptiveEngine:
    ...
    def get_first_question(self, db: Session) -> Optional[models.QuestionBank]:
        ...
        question = db.query(models.QuestionBank).filter(
            models.QuestionBank.category == "Brushing Habits",
    ...
    def get_next_question(
        ...
        current_category = assessment.current_category or "Brushing Habits"
        ...
        if asked_ids:
            query = query.filter(models.QuestionBank.id.notin_(asked_ids))
        next_q = query.order_by(models.QuestionBank.id).first()

        # Fallback: if no questions in current category/difficulty, try same category with any difficulty
        if not next_q:
            query_fb = db.query(models.QuestionBank).filter(
                models.QuestionBank.category == new_category
            )
            if asked_ids:
                query_fb = query_fb.filter(models.QuestionBank.id.notin_(asked_ids))
            next_q = query_fb.order_by(models.QuestionBank.id).first()

        # Fallback: if no questions in category, move to ANY other available question
        if not next_q:
            query_any = db.query(models.QuestionBank)
            if asked_ids:
                query_any = query_any.filter(models.QuestionBank.id.notin_(asked_ids))
            next_q = query_any.order_by(models.QuestionBank.id).first()
            if next_q:
                new_category = next_q.category
                new_difficulty = next_q.difficulty
        ...
        if next_q:
            ...
            # Track this question as asked using assignment for SQLAlchemy JSON tracking
            asked = list(assessment.asked_questions) if isinstance(assessment.asked_questions, list) else []
            if next_q.id not in asked:
                asked.append(next_q.id)
            assessment.asked_questions = asked
    ...
    def _adapt_category(
        ...
        if answered_count < 3:
            return "Brushing Habits"

        # Warmup extension: if still struggling after warmup, extend Brushing
        if current_category == "Brushing Habits" and wrong_streak > 2:
            return "Brushing Habits"
